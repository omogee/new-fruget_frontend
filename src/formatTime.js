...
if(currentYear === messageYear && currentMonth === messageMonth && currentDay === messageDay){
  ...
  if(currseconds < 60){
   return `${Math.floor(currseconds)} sec`
  }
  else if(currseconds > 60 && currseconds < 3600){
   return `${Math.floor(currseconds/60)} min`
  }
  else if(currseconds > 3600 && currseconds < 86400){
   return `${Math.floor(currseconds/3600)} hrs`
  }
  else if(currseconds > 86400 && currseconds < 604800){
      return `${Math.floor(currseconds/86400)} days`
  }
  else if(currseconds > 604800){
       return `${Math.floor(currseconds/604800)}w`
  }
  else {
       return null
  }
...
export function formateronlymain(time){
     const dd = new Date(parseInt(time))
const currseconds = (currentTime - parseInt(time))/1000
const currentYear = d.getFullYear();
const currentMonth =d.getMonth();
  ...
  if(currseconds < 60){
   ...
   return ` ${Math.floor(currseconds/3600)} hrs`
      ...
      return ` ${Math.floor(currseconds/86400)} days`
       ...
       return ` ${Math.floor(currseconds/604800)} weeks`
  }
  else {
       return null
  }
...
if(currentYear === messageYear && currentMonth === messageMonth && currentDay === messageDay){
   ...
   return `yesterday`
     ...
     return `${dd.toLocaleString('default', { month: 'short' })},${messageDay} ${messageYear}`
    }
}